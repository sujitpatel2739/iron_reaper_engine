...
class ResBlock(Layer):
    ...
    def _backward(self, grad):
        ...
        # Post-Activation LayerNorm
        if self.lnorm_mode == 'post':
            grad = self.lnorm.backward(grad)

        # The gradient splits at the addition (Z = S + F)
        grad_s = grad
        grad_f = grad

        # -------------------------------------------
        # Backpropagate through Residual Path (F)
        # -------------------------------------------
        ...
        # -------------------------------------------
        # Backpropagate through Shortcut Path (S)
        # -------------------------------------------
        if self.shortcut:
            grad_s = self.shortcut.backward(grad_s)

        # Sum gradients at the split point
